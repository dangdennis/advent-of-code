/* https://adventofcode.com/2019/day/6
    --- Day 6: Universal Orbit Map ---
      ...
      only partly shown. In the map data, this orbital relationship is written AAA)BBB, which means "BBB is in orbit around AAA".
...
let parseLineToMap = line => {
  let map = Js.Dict.empty();
  let orbital = Js.String.split(")", line);
  if (Array.length(orbital) === 2) {
    switch (orbital[0]) {
    | None => ()
    | Some(key) => Js.Dict.set(map, key, orbital[1])
    };
  };
  map;
};

let countDirectAndIndirectOrbits = input => {
  let orbitalLines = Utils.multiLineToList(input);
  let orbitalMaps = List.map(orbitalLines, parseLineToMap)->List.toArray;
  let tree = Tree.createTree(orbitalMaps);

  Tree.walk(tree, node => Js.log(node));
  //   Js.log(tree);
};
